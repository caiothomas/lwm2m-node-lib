...
function register(host, port, url, endpointName, callback) {    
    ...
    function createAgent(options, callback){   
        ...
        if(config.client && config.client.authenticate == true && config.client.dtls_opts){                        
            ...
            new coap.Agent({type: 'udp4'}, _dtls, function(ag) {
                console.log(_dtls);
    ...
    function sendRequest(payload, req,  innerCallback) {
       ...
       req.on('response', createRegisterResponseHandler(deviceInformation, innerCallback));
        /*req.on('response', function(error){
            console.log("responsta", error.payload.toString("utf8"))
            console.log(error);
        });*/
    ...
    function execute(payload, callback){
        createAgent(creationRequest, function (req){
            sendRequest(payload, req, callback);
        })
    }
