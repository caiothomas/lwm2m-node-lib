 ...
 */
function startListener(deviceInformation, socket,  callback) {
    ...
    function startCoapServer(socket, innerCallback) {
        try {
            coapRouter.start(config.client, socket, innerCallback);
        } catch(error) {
            innerCallback(error);
        }
    }
    ...
    return function responseHandler(res) {
        ...
        if (res.code.match(/^2\.\d\d$/)) {            
            logger.debug(context, 'Registration oparation succeeded to host [%s] and port [%s] with code [%s]',
                deviceInformation.currentHost, deviceInformation.currentPort, res.code);
...
function register(host, port, url, endpointName, callback) {    
    ...
    if (config.client && config.client.logLevel) {
        logger.setLevel(config.client.logLevel);
    }
    
    function createAgent(options, callback){   
        var req = null;        
        if(config.client && config.client.authenticate == true && config.client.dtls_opts){                        
            var _dtls = {
              host: options.host,
              port: options.port || 5684
            };
            Object.assign(_dtls, config.client.dtls_opts);
            console.log("dtls:", _dtls);  
            
            new coap.AgentSocket({type: 'udp4'}, _dtls, null, function(ag) {
                req = ag.request(options, _dtls);                                
                callback(req, ag._sock);
            });                        
        } else {          
            var ag = new coap.Agent({type: 'udp4'});
            req = ag.request(options);
            callback(req, null);
        } 
    }
        
    function sendRequest(payload, req,  innerCallback) {
        logger.debug(context, 'Sending registration request', payload);
    ...
    function execute(payload, callback){
        createAgent(creationRequest, function (req, socket){
            console.log("payload ", payload)
            console.log("req", req.toString("utf-8"))
            sendRequest(payload, req, function(error){
    ...
    function sendRequest(payload, req,  innerCallback) {
        ...
        req.on('response', function (res){                           
            async.waterfall([
                ...
                apply(createRegisterResponseHandler(deviceInformation, innerCallback), res)
            ], innerCallback);
        });
...
function unregister(deviceInformation, callback) {
    ...
    function sendRequest(req,  innerCallback) {   
        req.on('response', function(res) {
    ...
    createAgent(creationRequest, function (req, socket){
        sendRequest(req, function(error){  
            if(socket){
                socket._closeSocket();                
            }
            callback(null)
