 ...
 */
function read(deviceId, objectType, objectId, resourceId, globalServerInfo, callback) {
    function createReadRequest(obj, callback) {
        var request = {
            host: (config.ipProtocol === 'udp6')?'::1':'127.0.0.1',
            port: config.port,
            method: 'GET',
            proxyUri: coapUrl() + (config.ipProtocol === 'udp6' ? '['+obj.address+']' : obj.address) + ':' + obj.port,
    ...
    async.waterfall([
        ...
        createReadRequest,
        apply(coapUtils.sendRequest, globalServerInfo),
 ...
 */
function writeResource(method, deviceId, objectType, objectId, resourceId, value, globalServerInfo, callback) {
 ...
 */
function write(deviceId, objectType, objectId, resourceId, value, globalServerInfo, callback) {
    writeResource('PUT', deviceId, objectType, objectId, resourceId, value, globalServerInfo, callback);
}

/**
 * Makes an Execute operation over the designed resource ID of the selected device.
 */
function execute(deviceId, objectType, objectId, resourceId, value, globalServerInfo, callback) {
    writeResource('POST', deviceId, objectType, objectId, resourceId, value, globalServerInfo, callback);
}
 ...
 */
function writeAttributes(deviceId, objectType, objectId, resourceId, attributes, globalServerInfo, callback) {
    ...
    function createWriteAttributesRequest(data, innerCb) {
        var request = {
            host: (config.ipProtocol === 'udp6')?'::1':'127.0.0.1',
            port: config.port,
            proxyUri: coapUrl() + (config.ipProtocol === 'udp6' ? '['+data[0].address+']' : data[0].address) +
              ':' + data[0].port,
            method: 'PUT'
        };
 ...
 */
function discover(deviceId, objectType, objectId, resourceId, globalServerInfo, fullCallback) {
    var pathname,
        trueCallback,
        serverInfo = null;
    
    if (isPresent(objectId, resourceId, globalServerInfo, fullCallback)) {
    ...
    } else if (isPresent(objectId, resourceId, globalServerInfo)) {
    ...
    if (!objectType && !objectId && !resourceId) {
        ...
        async.waterfall([
            apply(registry.get, deviceId),
            createReadRequest,
            apply(coapUtils.sendRequest, serverInfo),
            coapUtils.generateProcessResponse(objectType, objectId, resourceId, '2.05')
        ], trueCallback);
    }
}

function create(deviceId, objectType, objectId, globalServerInfo, callback) {
    function createUpdateRequest(obj, callback) {
        var request = {
            host: (config.ipProtocol === 'udp6')?'::1':'127.0.0.1',
            port: config.port,
...
function coapUrl(){
    if(config && config.authenticate == true && config.dtls_opts){   
        return 'coaps://';
    } else {
        return  'coap://';
    }
