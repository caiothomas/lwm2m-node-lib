...
function ResourceListener(deviceId, objectType, objectId, resourceId, observeStream, handler) {
    return {
        ...
        dataHandler: function (chunk) {
            logger.debug(context, 'New data on resource /%s/%s/%s in device [%d]',objectType, objectId, resourceId, deviceId);
 ...
 * @param {Function} handler            Handler to be called each time a new piece of data arrives.
 */
function observe(deviceId, objectType, objectId, resourceId, handler,globalServerInfo, callback) {
...
function cancel(deviceId, objectType, objectId, resourceId, callback) {
    ...
    if(subscriptions[subscriptionId]){
        console.log("        subscriptions[subscriptionId].stream",         subscriptions[subscriptionId].stream)
