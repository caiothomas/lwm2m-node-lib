...
function dataHandler(serverInfo) {
    return function(req, res) {
        ...
        if(req)
        //    console.log("dataHandler REQ", req);
        if(res)
          //  console.log("dataHandler REPS", res);
        ...
        if (req.code === '0.00' && req._packet.confirmable && req.payload.length === 0) {
            ...
            console.log("serverInfo.routes ", serverInfo.routes);
 ...
 */
function startCoap(config, socket, callback) {
    var serverInfo = {
        server: null,
        routes: [],
        handlers: null
    ...
    if(config.authenticate == true && config.dtls_opts_server){
        console.log('config.dtls_opts', config.dtls_opts_server);
        if(socket){
            console.log("Use socket Open")
            socket.removeAllListeners('data');
            socket.removeAllListeners('error');
            socket.removeAllListeners('close');
            
            serverInfo.server = libcoap.createServerSocket ({
                    port: config.port ,
                    type: "udp4",
                    proxy: false,
                    dtls: true
            ...
            console.log("Use new Socket")            
            serverInfo.server = libcoap.createServerSocket(
              {
                dtls: config.dtls_opts_server,
...
*/    
    ...
    serverInfo.server.listen(function (error) {
        if (error) {
